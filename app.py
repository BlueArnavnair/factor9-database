...
with nav_explorer:
    ...
    # FLEXIBLE MISSENSE FILTER (Supports p.Val92Ala, Arg > Gln, and raw mutation text)
    if only_missense:
        filtered_df = filtered_df[
            filtered_df['HGVS_p'].astype(str).str.contains('p\.|>', regex=True, na=False) |
            filtered_df['Mutation'].astype(str).str.contains('>', na=False)
        ]
    ...
    def make_label(row):
        ...
        if hgvs and hgvs not in ['-', 'nan', 'N/A']:
            return f"{hgvs} (Residue {res})"
        if mut and mut not in ['-', 'nan', 'N/A']:
            return f"{mut} (Residue {res})"
        return f"Residue {res}"
    ...
    disp_title = variant_row.get('HGVS_p') if pd.notnull(variant_row.get('HGVS_p')) and str(variant_row.get('HGVS_p')) != 'nan' else variant_row.get('Mutation', 'N/A')
    st.subheader(f"Variant Profile: {disp_title}")
    ...
    with tab_cards:
        st.markdown(f"### Mechanism Breakdown for `{disp_title}`")
